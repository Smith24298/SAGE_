...
import { fetchChat, getEmployeeSummary } from '@/lib/api';
import {
  personalizeResponse,
    ...
    try {
      ...
      if (isMeetingPrepQuestion(input)) {
        ...
        if (employeeName) {
          ...
          if (!employeeSummary) {
            try {
              ...
              if (employeeSummary) {
                console.log(`Got summary for ${employeeName}:`, employeeSummary);
                setEmployeeCache((prev) => ({
                  ...prev,
                  [employeeName.toLowerCase()]: employeeSummary,
                }));
              } else {
                console.log(`No summary found for ${employeeName}, using generic response`);
              }
            } catch (error) {
              console.error(`Error fetching summary for ${employeeName}:`, error);
              // Fall through to generic response
            }
          }

          // Generate personalized meeting prep response
          assistantResponse = generateMeetingPrepResponse(employeeName, employeeSummary);
        } else {
          assistantResponse = 'Who would you like to meet with? (e.g., "prepare me for a meeting with Alice")';
        }
      } else {
        ...
        try {
          ...
          console.log('Chat response:', assistantResponse.substring(0, 100) + '...');
        } catch (error) {
