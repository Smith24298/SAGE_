...
import { TrendingUp, TrendingDown, AlertCircle, Heart, ArrowLeft } from 'lucide-react';
      ...
      <div className="flex items-center gap-4">
        <Link href="/departments" className="block">
          <motion.div 
            ...
            whileTap={{ scale: 0.95 }}
            className="flex items-center justify-center w-10 h-10 rounded-xl bg-primary/10 text-primary hover:bg-primary hover:text-primary-foreground transition-colors shadow-sm border border-primary/20"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        ...
        <motion.div ref={ref} variants={scrollVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-60px' }} transition={{ duration: 0.5, delay: 0.2 }}>
          <Card className="p-6 shadow-md">
            ...
            <ResponsiveContainer width="100%" height={250}>
              <BarChart key={animKey} data={dept.subteamAnalytics}>
                <defs>
            ...
            <div className="flex-1">
              ...
              <div className="space-y-4">
                <div className="bg-background/60 p-4 rounded-lg">
                  ...
                  <p className="mt-1 text-sm">{dept.aiInsight}</p>
                </div>
                <div className="bg-primary/10 border border-primary/20 p-4 rounded-lg">
                  <span className="text-xs uppercase font-bold text-primary">Recommendation</span>
                  <p className="mt-1 text-sm font-medium">{dept.aiRecommendation}</p>
                </div>
              </div>
            </div>
          </div>
